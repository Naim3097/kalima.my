/*
  Refuse an order the shop cannot fulfil, before the customer pays for it.

  create_order priced and recorded anything published, whatever the stock. The
  oversell guard lived only in mark_order_paid, which decrements conditionally
  and raises when the row count is zero — correct, but it fires AFTER the money
  has moved. The customer is charged, the webhook throws, the route 500s, and
  the gateway retries a call that can never succeed while the order sits
  pending. Two orders for 0-stock items already exist from testing.

  So this is the polite gate and mark_order_paid stays the real one. Nothing
  here reserves stock: a variant with one unit left can still pass this check
  in two concurrent carts. The authoritative decision remains the atomic
  decrement at settlement, which is the only place that can be race-free
  without holding a lock across the customer's trip to the bank.

  The error names the product, the colour and the size, because "out of stock"
  on a nine-field checkout tells a shopper nothing about which line to remove.
*/
...
begin
  ...
  for item in select * from jsonb_array_elements(p_items) loop
    ...
    /*
      Advisory stock gate. Not a reservation — see the note above — but it
      stops a shopper paying for something that is already gone.
    */
