/*
  ...
  - order/payment prices are SNAPSHOTS taken server-side at creation, so the
  ...
  - an order becomes 'paid' ONLY through mark_order_paid (a verified webhook),
    which is idempotent and oversell-guarded

  Note: ids default to gen_random_uuid() (pg_catalog) rather than
  uuid_generate_v4() (extensions schema) so the SECURITY DEFINER functions in
  the companion migration, which pin search_path='', can insert into these
  tables without the extension being on the path.
*/
...
create table stock_movements (
  ...
  qty_delta          integer not null,   -- negative = out, positive = in
...
create table discount_codes (
  ...
  amount          integer not null default 0,  -- percent: 0..100; fixed: sen off
...
-- Signed-in customers see their own orders; staff see all. Guest orders
