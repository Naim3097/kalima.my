/*
  Move every order function behind service_role, invoked only from server-side
  app code (checkout server action, confirmation page, webhook). This removes
  the anon/authenticated RPC surface for order creation, discount probing and
  order lookup.

  create_order gains an explicit p_user_id: called via the service client,
  auth.uid() is null, so the verified caller's id must be passed in by the
  server action (which reads it from the authenticated session, never trusting
  client input). Null = guest.
*/
drop function public.create_order(jsonb, text, text, jsonb, text, text);
...
-- All order functions: server-side (service_role) only.
revoke all on function public.create_order(uuid, jsonb, text, text, jsonb, text, text) from public;
