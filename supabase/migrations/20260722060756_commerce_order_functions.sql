/*
  Commerce entry-point functions. All SECURITY DEFINER with a pinned
  search_path; the function body is the trust boundary.

  FREE_SHIPPING_THRESHOLD = RM300 (30000 sen); flat shipping RM10 (1000 sen)
  below it. This is a placeholder until EasyParcel live rates (Phase 4).
*/

-- Returns the discount (in sen) a code yields for a given subtotal, plus a
-- normalized reason if it does not apply. Reads discount_codes as definer so
-- shoppers can validate a code without read access to the table.
...
begin
  ...
  if p_subtotal_sen < c.min_spend_sen then
    return jsonb_build_object('valid', false,
      'reason', 'Spend at least RM' || (c.min_spend_sen / 100) || ' to use this code',
      'discount_sen', 0);
  end if;

  if c.kind = 'percent' then
    discount := (p_subtotal_sen * c.amount) / 100;
  elsif c.kind = 'fixed' then
    discount := least(c.amount, p_subtotal_sen);
  else
    discount := 0;  -- free_shipping affects shipping, handled in create_order
  end if;

  return jsonb_build_object(
    'valid', true, 'code', c.code, 'kind', c.kind,
...
/*
  Creates a PENDING order from a cart. Prices, discount and shipping are all
  computed here from the database — nothing about money is trusted from the
  caller. Items arrive as [{variant_id, qty}, ...].

  Does NOT touch stock: reservation happens at payment (mark_order_paid), so an
  abandoned pending order never holds inventory. Availability is best-effort
  validated here; the authoritative oversell guard is at payment.

  Returns { order_id, reference, total_sen }.
*/
...
begin
  ...
  for item in select * from jsonb_array_elements(p_items) loop
    ...
    -- Variant price override, else the product base price. Server-authoritative.
  ...
  -- Shipping: free over RM300 or with a free-shipping code, else flat RM10.
  ...
  -- Second pass: snapshot each line item.
  for item in select * from jsonb_array_elements(p_items) loop
    line_qty := (item ->> 'qty')::integer;
  ...
  -- Record the intended redemption (counted for real at payment).
...
/*
  Confirms payment: idempotent + oversell-guarded, all in one transaction.
  Called ONLY by the payment webhook (service_role). Steps:
    1. no-op if the order is already paid (webhooks retry)
    2. upsert the payment row
    3. decrement each variant's stock with a guard, logging a ledger row
    4. flip the order to 'paid', bump discount redemption counts
  If any line can't be fulfilled, the whole thing rolls back and the order
  stays pending for reconciliation.
*/
...
begin
  ...
  -- Oversell-guarded decrement + ledger row per line.
  for it in select * from public.order_items where order_id = p_order_id loop
    ...
    if updated = 0 then
      raise exception 'insufficient stock for %', it.variant_sku;
    end if;
...
/*
  ...
  as jsonb only when the email matches (guests have no session, so RLS can't
  key on auth.uid()). Signed-in users still use RLS-backed reads.
*/
...
-- Grants: shopper-facing entry points to anon + authenticated; the
-- payment confirmation strictly to service_role (webhook only).
