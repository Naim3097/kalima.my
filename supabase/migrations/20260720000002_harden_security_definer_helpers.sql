/*
  Security hardening — clears three database-linter warnings raised after the
  catalog migration.
  ...
  1. public.is_staff() was reachable as an RPC endpoint (/rest/v1/rpc/is_staff)
     ...
     to work at all. The fix is to move it into a `private` schema, which
     PostgREST does not expose, and repoint the policies at it.

  2. set_updated_at() had a role-mutable search_path.
*/
...
-- Recreate policies against the relocated helper. is_staff() stays wrapped in a
-- scalar subquery so it is evaluated once per query rather than once per row.
