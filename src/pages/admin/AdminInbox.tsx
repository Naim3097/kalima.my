...
import { Card, DemoNote } from "../../components/admin/ui";
      ...
      <div className="flex flex-wrap gap-2">
        <button
          ...
          className={`label-caps px-4 py-2 transition-colors cursor-pointer ${
            filter === "all" ? "bg-navy text-white" : "border border-navy/20 text-navy-400 hover:border-navy hover:text-navy"
          }`}
        >
        ...
        {(Object.keys(INBOX_CHANNEL_META) as InboxChannel[]).map((ch) => (
          <button
            ...
            className={`label-caps px-4 py-2 transition-colors cursor-pointer ${
              filter === ch ? "bg-navy text-white" : "border border-navy/20 text-navy-400 hover:border-navy hover:text-navy"
            }`}
          >
              ...
              return (
                <li key={c.id}>
                  <button
                    ...
                    className={`w-full px-4 py-3.5 text-left transition-colors cursor-pointer ${
                  ...
                  >
                    ...
                    <p className={`mt-1 truncate text-[12px] ${c.unread ? "font-medium text-navy" : "text-navy-400"}`}>
                      {c.preview}
                    </p>
        ...
        <div className="flex flex-col border-b border-navy/10 xl:border-b-0 xl:border-r">
          <div className="flex items-center justify-between border-b border-navy/10 px-5 py-3.5">
            <div className="flex items-center gap-2.5">
              ...
              <span className="rounded px-2 py-0.5 text-[10px] font-medium uppercase tracking-wider text-white" style={{ background: activeMeta.color }}>
                {activeMeta.label}
              </span>
            </div>
            <button className="label-caps text-navy-400 hover:text-navy transition-colors cursor-pointer">Assign</button>
          </div>
              ...
              <div key={i} className={`flex ${m.from === "kalima" ? "justify-end" : "justify-start"}`}>
                <div
                  className={`max-w-[75%] rounded-lg px-4 py-2.5 text-[13px] leading-relaxed ${
                    m.from === "kalima" ? "rounded-br-none bg-navy text-white" : "rounded-bl-none bg-navy-100 text-navy"
                  }`}
                >
                  {m.text}
                  <p className={`mt-1 text-[10px] ${m.from === "kalima" ? "text-white/50" : "text-navy-300"}`}>{m.time}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="border-t border-navy/10 p-4">
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 border border-navy/20 bg-cream-50 px-4 py-2.5 text-[13px] text-navy placeholder:text-navy-300 focus:border-navy focus:outline-none"
              />
              <button className="label-caps bg-navy px-5 text-white hover:bg-navy-700 transition-colors cursor-pointer">
                Send
              </button>
            </div>
            <div className="mt-2.5 flex gap-2">
              {["Order status 📦", "Size guide 📏", "Restock alert 🔔"].map((quick) => (
                <button
                  key={quick}
                  onClick={() => setDraft(quick)}
                  className="rounded-full border border-navy/15 px-3 py-1 text-[11px] tracking-wide text-navy-400 hover:border-navy hover:text-navy transition-colors cursor-pointer"
                >
                  {quick}
                </button>
              ))}
        ...
        <div className="px-5 py-5">
          ...
          {active.linkedOrder ? (
            <div className="mt-4 border border-navy/10 bg-cream-50 px-4 py-3">
              ...
              <Link to="/admin/orders" className="mt-2 inline-block text-[12px] text-navy underline underline-offset-4">
                Open order →
              </Link>
            </div>
          ) : (
